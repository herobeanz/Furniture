<template>
  <div class="cms-list">
    ...
    <div v-else-if="items.length === 0" class="empty-state">Chưa có trang CMS.</div>
        ...
        <tbody>
          <tr v-for="p in paginatedItems" :key="p.id">
            ...
            <td class="actions-cell">
              <RouterLink :to="'/admin/cms-pages/' + p.id" class="action-btn edit" title="Sửa">
                ✏️
              </RouterLink>
...
async function handleBulkDelete() {
  ...
  if (!confirm(`Xóa ${selectedItems.value.length} trang CMS đã chọn?`)) return
  try {
    await Promise.all(selectedItems.value.map((id) => apiClient.delete(`/cms/${id}`)))
...
async function remove(id: number) {
  if (!confirm('Xóa trang CMS này?')) return
  try {
