  ...
  <div class="rooms-list">
    <div class="list-header">
      ...
      <div class="header-actions">
        ...
        <RouterLink to="/admin/rooms/new" class="btn-add">
          <span class="btn-icon">+</span>
          Thêm phòng mới
        </RouterLink>
      </div>
    ...
    <div v-else-if="items.length === 0" class="empty-state">Chưa có phòng.</div>
      ...
      <table class="data-table">
        ...
        <tbody>
          <tr v-for="r in paginatedItems" :key="r.id">
            <td>
              <input type="checkbox" :checked="selectedItems.includes(r.id)" @change="toggleSelect(r.id)" />
            </td>
            <td>{{ r.name }}</td>
            ...
            <td class="actions-cell">
              <RouterLink :to="'/admin/rooms/' + r.id" class="action-btn edit" title="Sửa">
                ✏️
              </RouterLink>
              <button type="button" class="action-btn delete" @click="confirmRemove(r)" title="Xóa">
                🗑️
              </button>
            </td>
          </tr>
...
const allSelected = computed(() => {
  ...
  return items.value.every((item) => selectedItems.value.includes(item.id))
})
...
async function handleBulkDelete() {
  ...
  if (!confirm(`Xóa ${selectedItems.value.length} phòng đã chọn?`)) return
  try {
    await Promise.all(selectedItems.value.map((id) => apiClient.delete(`/rooms/${id}`)))
